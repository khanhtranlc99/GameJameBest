...
namespace Game.Character.Config
{
	public abstract class Config : MonoBehaviour
	{
		...
		{
			Dictionary<string, Param> value;
			Param value2;
			if (Params.TryGetValue(mode, out value) && value.TryGetValue(key, out value2))
			{
						...
						if (dictionary2 != null)
						{
							foreach (KeyValuePair<string, object> item2 in dictionary2)
							{
								...
								else
								{
									...
									ConfigValue configValue = (ConfigValue)(int)Enum.Parse(typeof(ConfigValue), array[0] as string);
									Param param = null;
									switch (configValue)
									{
