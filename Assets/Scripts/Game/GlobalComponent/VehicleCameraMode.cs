...
namespace Game.GlobalComponent
{
	...
	public class VehicleCameraMode : CameraMode
	{
		...
		private void RotateCamera()
		{
			...
			//UnityEngine.Debug.LogError("Horizontal_R : "+Controls.GetAxis("Horizontal_R") +" Vertical_R : "+Controls.GetAxis("Vertical_R") );
			...
			if (rotationInput)
			{
				...
				//UnityEngine.Debug.LogError("gau here");
			}
			...
			//UnityEngine.Debug.LogError("RotationSpeedY" +config.GetFloat("RotationSpeedY") + "RotationSpeedX"+config.GetFloat("RotationSpeedX"));
			float num = (0f - rotY) * 57.29578f;
			//var rotateAxisLegit =0f;
			num =  Mathf.Clamp(num,config.GetFloat("RotationYMin"),config.GetFloat("RotationYMax"));
			// float @float = ;
			// float float2 = ;
			// if (num > @float)
			// {
			...
			// }
			rotY = (0f - num) * ((float)System.Math.PI / 180f);
		}
		...
		private void UpdateAutoReset()
		{
			...
			if (resetTimeout <= 0f)
			{
				...
				Vector3 dir;
				Vector3 dir2;
				Game.Character.Utils.Math.ToCartesian(num, rotZ, out dir);
